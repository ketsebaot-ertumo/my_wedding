...
//     setWebsiteUrl('https://azaria-ketsi-wedding.vercel.app/en');
//   }, []);
...
//                 className="bg-gradient-to-r from-rose-500 to-pink-500 hover:from-rose-600 hover:to-pink-600 cursor-pointer"
//               >
...
export default function QRCodeSection() {
  ...
  const downloadQRCode = () => {
    ...
    if (canvas) {
      ...
      const downloadLink = document.createElement('a')
      downloadLink.href = pngUrl
      downloadLink.download = 'azaria-ketsi-wedding-qr.png'
      document.body.appendChild(downloadLink)
      downloadLink.click()
      document.body.removeChild(downloadLink)
      toast.success('QR Code downloaded!')
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6">
      <div className="glass-effect rounded-2xl py-8 sm:p-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 mb-4">
            <Smartphone className="w-8 sm:w-12 h-8 sm:h-12 text-rose-500" />
            <h2 className="text-3xl md:text-5xl font-bold text-gray-800">{t('qr-code-title')}</h2>
          </div>
